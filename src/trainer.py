...
from src.model.model import LSTUR
...
class Trainer(BaseTrainer):
    ...
    def train(self):
        ...
        news_encoder = NewsEncoder.from_pretrained(args.pretrained_embedding, config=config,
                                                   ...
                                                   pad_category_id=self._category2id['pad'], query_dim=args.query_dim,
                                                   dropout=args.dropout, category_embed_dim=args.category_embed_dim,
                                                   category_embed=category_embed, num_category=len(self._category2id),
                                                   num_cnn_filters=args.num_cnn_filters, window_size=args.window_size,
                                                   word_embed_dim=args.word_embed_dim)
        user_encoder = UserEncoder(news_encoder=news_encoder, num_users=len(self._user2id),
                                   pad_user_id=self._user2id['pad'], num_gru_layers=args.num_gru_layers,
                                   gru_dropout=args.gru_dropout, long_term_dropout=args.long_term_dropout,
                                   combine_type=args.combine_type)
        model = LSTUR(user_encoder=user_encoder, news_encoder=news_encoder)
    ...
    def _forward_step(model, batch):
        logits = model(user_encoding=batch['user_encoding'], title=batch['title'], title_mask=batch['title_mask'],
                       his_title=batch['his_title'], his_title_mask=batch['his_title_mask'], his_mask=batch['his_mask'],
